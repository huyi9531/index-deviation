 ...
 * 也不会打线上接口。所以可以随便重复跑。
 *
 * 覆盖九个分支（这些分支靠等线上真实状态变化是等不到的）：
 *   ① 首次运行（KV 无状态）→ 必须发「监控已启用」，计数 = 1
 *   ② 状态未变            → 必须发「再次提醒」，计数 = 2
 *   ③ 状态仍未变          → 必须静默（同一件事最多 2 次）
 *   ④ 新增触发            → 必须发「新触发」，**计数归零重算**
 *   ⑤ 状态未变            → 「再次提醒」到第 2 次
 *   ⑥ 接口响应结构异常     → 必须抛错且**不写 KV**（否则这批变化被吞掉，永远不再通知）
 *   ⑦ 回到 ⑤ 的状态       → 必须静默 → 反证 ⑥ 没写 KV（否则计数会归零而重新提醒）
 *   ⑧ 全部解除            → 必须发「已解除」，计数归零
 *   ⑨ 状态未变且无触发     → 必须静默（不提醒「当前触发 0 个」这种纯噪声）
 *
...
const scenarios = [
  {
    ...
    worker: [/已投递（第 1\/2 次）.*监控已启用/],
    pushes: 1,
  },
  {
    name: '② 状态未变 → 第 2 次（再次提醒）',
    payload: wrap([STAR50_HIT, SP500]),
    http: 200,
    worker: [/已投递（第 2\/2 次）.*再次提醒/],
    pushes: 1,
  },
  {
    name: '③ 状态仍未变 → 静默（已达上限 2 次）',
    payload: wrap([STAR50_HIT, SP500]),
    http: 200,
    worker: [/已推 2 次（上限 2），静默/],
    pushes: 0,
  },
  {
    name: '④ 新增触发 hstech → 新消息，计数归零',
    payload: wrap([STAR50_HIT, HSTECH_HIT, SP500]),
    http: 200,
    worker: [/已投递（第 1\/2 次）.*新触发 1 个/],
    pushes: 1,
  },
  {
    name: '⑤ 状态未变 → 再次提醒到第 2 次',
    payload: wrap([STAR50_HIT, HSTECH_HIT, SP500]),
    http: 200,
    worker: [/已投递（第 2\/2 次）.*再次提醒/],
    pushes: 1,
  ...
  {
    name: '⑦ 回到 ⑤ 的状态 → 静默（反证 ⑥ 没写 KV）',
    payload: wrap([STAR50_HIT, HSTECH_HIT, SP500]),
    http: 200,
    // 若 ⑥ 误写了 KV，计数会被归零，这里就会变成「再次提醒」而不是静默
    worker: [/已推 2 次（上限 2），静默/],
    pushes: 0,
  },
  {
    ...
    payload: wrap([SP500]),
    http: 200,
    worker: [/已投递（第 1\/2 次）.*已解除 2 个/],
    pushes: 1,
  },
  {
    name: '⑨ 状态未变且无触发 → 静默（不提醒「0 个」）',
    payload: wrap([SP500]),
    http: 200,
    worker: [/当前无触发，不提醒/],
    pushes: 0,
