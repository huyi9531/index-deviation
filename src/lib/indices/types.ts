...
export interface AnalogAnswer {
  ...
  episodes: number
  /**
   * 同类位置 20 日胜率（**交易日加权**：带内每一天都算）。
   * 不用「每段独立信号只取首次触达日」：首次触达日永远是那一段里**最浅**的一天，
   * 历史上比段内更深的那些日子差 —— 那个口径回答的是「恰好在穿越那天买入」，
   * 与用户「被告知当天或某天打开时动手」不是同一个问题（实测能翻转 10 格里
   * 5~6 格的样本外结论）。段数只用于置信区间的样本量。
   */
  win20: number
   ...
   * 点估计是交易日加权的，而区间用的样本量是**独立信号段数**（同一段内的观测
   * 高度相关，拿交易日当 n 会把区间压得假窄）—— 这是聚类数据的标准保守处理。
   */
