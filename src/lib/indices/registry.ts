 ...
 * 目前 20 个格子里只剩纳斯达克100 的 dev200 仍是 null。
 *
 * 标定方法（两级）：
 *   ① 全样本口径：候选阈值中独立信号 ≥ 12 次、且 60 日胜率超额 ≥ +3pp 的**最浅**一档；
 *   ② 时代复核：① 的档位全部不达标时，改用「当前时代」窗口（基线也在窗口内算）
 *      复核一遍，达标则采用并加 † 标记 —— 表示该水位只在当前时代成立，
 *      不是跨 40 年都成立的结论。
 *
 * ② 是 2026-09 补的，因为全样本单口径对「结构性断代」的指数不公平：
 * 纳斯达克100 的深跌超额在 1986-1999（+7pp）、2010-2019（+7.5pp）、2020 至今
 * （+9pp）三段都为正，只有 2000-2010 那一代是负的（深跌后 60 日胜率 43%
 * vs 常态 53.6%），全样本一平均就归零 —— 「2000-2010 失效」不等于「当下没有优势」。
 * 但它不是「总能标出一个水位」的橡皮图章：那个时代窗口自己复核同样不达标，
 * 所以口径在熊市时代会自动收紧回 null。
 ...
 * † = 全样本无档位达标、由当前时代窗口复核得出，成立范围见下方 ⚠️）：
 *
 *   指数          dev60                     dev200
 *   标普500       -7%   (+3.5pp, 134 次)     -10%  (+3.2pp, 96 次)
 *   纳斯达克100    -10%  (+7.8pp, 27 次)†    无优势（全档位为负）
 *   沪深300       -3%   (+15.1pp, 50 次)†   -12%  (+5.1pp, 39 次)
 *   中证A500      -3%   (+14.9pp, 48 次)†   -12%  (+5.1pp, 36 次)
 ...
 *
 * ⚠️ 三个 † 格的成立范围（2026-09 新增，用「当前时代」窗口 + 窗口内基线标定）：
 *   纳斯达克100 dev60 -10%（2010 年后，27 段）—— 全样本 10 档全负，但同为「深跌」
 *     的位置在 2010 年后跑赢常态 7.8pp、均值超额也有 +9.5pp；反向证据是 2000-2010
 *     那一代深跌后胜率仅 43%，所以这条水位当时会失效（时代口径会自动收紧回 null）。
 *   沪深300 dev60 -3%、中证A500 dev60 -3%（均为 2019 年后，50 / 48 段）——
 *     这两个的 dev200 仍是全样本口径的老值，两格口径不同，不要横向当同一强度读。
 *   三格的统计强度都不强（独立信号单侧检验 p≈0.18 / 0.02 / 0.02），与已发布的
 *   标普500 dev200（p≈0.25）、日经225 dev200（p≈0.29）在同一量级 —— 整个水位体系
 *   本就处在「弱显著」水平，这是待统一提高阈值时要一起解决的问题，不是 NDX 独有。
 *
 * ⚠️ 全样本口径下「最浅档」有个已知未修的不一致：沪深300 / 中证A500 / 中证500 的
 *   dev200 与创业板指的两个口径，现存值都比现行规则算出来的更**深**
 *   （如沪深300 dev200 现值 -12%，按规则应取 -8%）。这些值在标称档位上能精确复现，
 *   属于历史选档规则与文档不一致，已在 .agents/plans 立项，等统一口径那一轮一起重标。
 *   本轮只补了三个空白格，**没有**改动任何已有水位。
 *
 * 读法：创业板指两个口径都很有效；沪深300 的全样本 60 日口径跑不赢常态（大盘股趋势性
 * 更强），只有切到 2019 年后才有 -3% 这一条 —— 所以它的两个格子口径不同、强度不可比。
 * 纳斯达克100 是反向例子：全样本一格都不给，切到当前时代才有一条 60 日水位（-10%），
 * 比标普的 -7% 深 —— 「要跌得更狠才算数」已经写在标定结果里了。
 *
 ...
 *   并连同已发布的全部水位（含本轮新增的 3 格：它们同样是「当前时代成立」口径）一起重标。
 *
...
export const INDICES: readonly IndexDef[] = [
  ...
  },
  {
    ...
    // dev60：全样本 10 档全负（-4% ~ -18% 每一档超额都在 -13 ~ -4pp 之间，是十个指数里
    // 唯一 20 格全负的），故 2026-09 之前这里是 null。现按「时代复核」取 2010 年后窗口的
    // -10%（+7.8pp / 27 段）—— 那条水位在 2000-2010 一代会失效，见标定结果表下的 ⚠️。
    // dev200 仍为 null：时代窗口里同样是全档位为负（-15.7 ~ -22.2pp），最深的 -20%
    // 只有 8 段独立信号、够不上 ≥12 段门槛，所以这一格如实空着，不硬凑。
  ...
  },
  {
    ...
    // dev60 -3% 来自「2019 年后」窗口复核（全样本 10 档全负），+15.1pp / 50 段；
    // dev200 -12% 是全样本口径的老值，保持不动。
    action: { dev60: -3, dev200: -12 },
  },
  {
    id: 'a500',
    market: 'cn',
    ...
    // 同沪深300：dev60 -3% 来自「2019 年后」窗口复核（+14.9pp / 48 段），
    // dev200 -12% 是全样本口径的老值。
    action: { dev60: -3, dev200: -12 },
  },
  {
